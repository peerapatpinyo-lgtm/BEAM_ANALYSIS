...
st.title("🏗️ Professional Beam Design & Analysis Studio")

# --- 1. GEOMETRY & ADVANCED LOADS ---
...
with st.container(border=True):
    ...
    with col_g2:
        ...
        l_x = c4.number_input("Start Pos (x)", 0.0, float(spans[l_span]))
        l_end = c5.number_input("End Pos (x)", float(spans[l_span])) if l_type == "Uniform (U)" else l_x
...
with st.sidebar:
    ...
    fy = st.number_input("fy Rebar (MPa)", 400.0)
...
if st.button("🚀 EXECUTE STRUCTURAL ANALYSIS", type="primary", use_container_width=True):
    ...
    if not df.empty:
        ...
        st.header("📋 PART II: Technical Calculation Sheet (SDM)")
        db_m = st.selectbox("Select Main DB", [12, 16, 20, 25, 28])
        
        cum_dist = [0] + list(np.cumsum(spans))
        for i in range(n_spans):
            with st.expander(f"SPAN {i+1} DETAILED REPORT", expanded=True):
                ...
                with c1:
                    ...
                    p_m = res['pos']['mu_val']
                    n_m = res['neg']['mu_val']
                    st.latex(rf"M_u^{(+)} = {p_m:.2f} \text{{ kNm}}")
                    st.latex(rf"M_u^{{(-)}} = {n_m:.2f} \text{{ kNm}}")
                ...
                with c3:
                    ...
                    n_top = st.number_input(f"Top Count S{i+1}", 2, 12, int(res['neg']['n']), key=f"t_{i}")
                    n_bot = st.number_input(f"Bot Count S{i+1}", 2, 12, int(res['pos']['n']), key=f"b_{i}")
