...
if st.session_state.results:
    ...
    if df is None or df.empty:
        st.error("❌ Solver returned empty data. Check inputs.")
    else:
        x_col = 'x' if 'x' in df.columns else df.columns[0]
        st.success("Analysis Complete!")
        
        # --- A. ANALYSIS GRAPHS (SFD & BMD) กลับมาแล้ว ---
        st.header("📈 Part 1: Force Diagrams")
        ...
        fig_main.update_yaxes(autorange="reversed", row=2, col=1) 
        
        # Key สำคัญมาก ป้องกันกราฟหาย
        st.plotly_chart(fig_main, use_container_width=True, key="analysis_graphs")


        # --- B. LONGITUDINAL ---
        st.header("🖼️ Part 2: Longitudinal View")
        ...
        for l in spans:
            ...
            cum_l += l
        fig_long.add_trace(go.Scatter(x=[cum_l], y=[-0.05], mode="markers", marker=dict(symbol="triangle-up", size=15, color="black")))
        fig_long.update_layout(height=200, showlegend=False, xaxis=dict(visible=True), yaxis=dict(visible=False, scaleanchor="x"))
        
        st.plotly_chart(fig_long, use_container_width=True, key="long_view")


        # --- C. DESIGN & SECTIONS ---
        st.header("📋 Part 3: Design & Sections")
        ...
        for i in range(n_spans):
            with st.container(border=True):
                ...
                with c1:
                    st.subheader(f"Span {i+1} Output")
