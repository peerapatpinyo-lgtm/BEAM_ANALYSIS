                    ...
                    for j in range(num_p):
                        with st.container():
                            ...
                            if p_dl != 0:
                                loads.append({
                                    'span_idx': i, 'type': 'P', 
                                    'P': p_dl * f_dl, 'raw': p_dl,
                                    'x': x_loc, 'source': 'DL'
                                })
                            if p_ll != 0:
                                loads.append({
                                    'span_idx': i, 'type': 'P', 
                                    'P': p_ll * f_ll, 'raw': p_ll,
                                    'x': x_loc, 'source': 'LL'
                                })
    return loads

# ==========================================
# 3. VISUALIZATION ENGINE (REFINED)
# ==========================================
...
def add_peak_box(fig, x, y, val, color, row, position="top"):
    """Adds a professional box annotation without arrow"""
    yshift = 25 if position == "top" else -25
    
    # Professional Label: Value + Location
    label_text = f"<b>{val:.2f}</b><br><span style='font-size:10px; color:#444'>@ {x:.2f} m</span>"
    
    fig.add_annotation(
        x=x, y=y,
        text=label_text,
        showarrow=False,  # No arrow as requested
...
def create_engineering_plots(df, vis_spans, vis_supports, loads, unit_sys):
    ...
    # --- 1. GLOBAL SCALING ---
    # Determine max load to scale visuals proportionally
    all_magnitudes = []
    for l in loads:
    ...
    target_h = 1.5 # Base visual height
    ...
    # Position Tracker to prevent overlap of Point Loads
    # Key: (Span_Idx, X_Rough_Location) -> Count
    point_load_tracker = {}
    ...
    for load in loads:
        ...
        # --- POINT LOAD (Separated & Offset) ---
            ...
            # --- OFFSET LOGIC ---
            # If multiple point loads exist at same X, shift them visually
            ...
            # Visual Shift (offset 2% of total length per overlapping load)
    ...
    if len(np_v) > 0:
        ...
        if abs(v_max) > 0.1: add_peak_box(fig, np_x[idx_max], v_max, v_max, shear_color, 2, "top" if v_max > 0 else "bottom")
        if abs(v_min) > 0.1: add_peak_box(fig, np_x[idx_min], v_min, v_min, shear_color, 2, "bottom" if v_min < 0 else "top")
    ...
    if len(np_m) > 0:
        ...
        if abs(m_max) > 0.1: add_peak_box(fig, np_x[idx_max], m_max, m_max, moment_color, 3, "top" if m_max > 0 else "bottom")
        if abs(m_min) > 0.1: add_peak_box(fig, np_x[idx_min], m_min, m_min, moment_color, 3, "bottom" if m_min < 0 else "top")
