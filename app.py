            ...
            for i in range(n_spans):
                ...
                with st.expander(f"📍 SPAN {i+1} (L={s_len} m)", expanded=True):
                    ...
                    with col_input:
                        ...
                        # ต้องมี stir_db สำหรับคำนวณ d
                        temp_stir_db = st.selectbox("Stirrup Size", [6, 9, 12], index=1, key=f"sdb_ref_{i}")
                        ...
                        with ct2: top_n = st.number_input("Qty", 2, 15, 2, key=f"tn_{i}")
                        with ct3: top_layers = st.selectbox("Layers", [1, 2, 3], index=0, key=f"tl_{i}")
                        
                        # คำนวณ d_effective ตามจำนวนชั้น (สมมติระยะห่างชั้น 25mm)
                        d_real_t = h_mm - (cover_mm + temp_stir_db + top_db/2 + (top_layers-1)*12.5)
                        ...
                        with cb2: bot_n = st.number_input("Qty", 2, 15, 3, key=f"bn_{i}")
                        with cb3: bot_layers = st.selectbox("Layers", [1, 2, 3], index=0, key=f"bl_{i}")
                        
                        d_real_b = h_mm - (cover_mm + temp_stir_db + bot_db/2 + (bot_layers-1)*12.5)
                        ...
                        # ใช้ d_real_b สำหรับคำนวณแรงเฉือน
                        status_v, phi_Vn, _, _, _, _ = rc_design_engine.check_shear_details(vu_max, b_mm, d_real_b, fc, fy, temp_stir_db, stir_s)
                    ...
                    with col_draw:
                        ...
                        cs_data = {
                            ...
                            'stir_db': temp_stir_db, 'shear': {'s': stir_s}
                        }
                    ...
                    final_design_res.append({
                        ...
                        'top_db': top_db, 'bot_db': bot_db, 'stir_db': temp_stir_db,
                        'pos': {'n': bot_n, 'area': as_prov_bot, 'layers': bot_layers},
                        'neg': {'n': top_n, 'area': as_prov_top, 'layers': top_layers},
                        'shear': {'s': stir_s, 'db': temp_stir_db, 'status': status_v},
