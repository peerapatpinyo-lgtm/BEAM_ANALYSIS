...
with c1:
    ...
    # FIX: Initialize with ALL keys to prevent KeyError
    ...
    # Display Current Supports (Safe Display)
    st.write("**Current Supports:**")
    if st.session_state.supports_list:
        df_sup = pd.DataFrame(st.session_state.supports_list)
        
        # Safety: Ensure 'settlement' column exists
        if 'settlement' not in df_sup.columns:
            df_sup['settlement'] = 0.0
...
if st.button("🚀 Calculate Analysis", type="primary"):
    if not spans:
        ...
        try:
            ...
            # Mark max deflection
            min_y = results['deflection'].min() # Downward is negative usually
