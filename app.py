...
if not stable:
    ...
    # ================= TAB 1: DIAGRAMS =================
    with tab1:
        st.info(f"💡 Analysis includes **Self-Weight** ({w_sw_kN:.2f} kN/m) automatically.")
        
        # Plot Diagrams
        # We pass calc_loads_df to show total load (including SW) on the diagram
        ...
        if R:
            ...
            r_data = [{"Node": k, "Force (kN)": v/1000} for k, v in R.items()]
            st.dataframe(pd.DataFrame(r_data).T)
