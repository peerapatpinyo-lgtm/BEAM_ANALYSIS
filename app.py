...
# ==========================================
# ENGINEERING PLOTTING (With Global X Fix)
# ==========================================
...
def draw_support_shape(fig, x, y, sup_type, size=1.0):
    ...
    if sup_type == "Pin":
        ...
        fig.add_shape(type="line", x0=x-s, y0=y-s*0.8-s*0.2, x1=x+s, y1=y-s*0.8-s*0.2, line=dict(color=line_col, width=2), row=1, col=1)
    elif sup_type == "Fixed":
        h_wall = s * 1.5
...
def create_engineering_plots(df, vis_spans, vis_supports, loads, unit_sys):
    ...
    # --- DATA PREP FOR PLOTTING (Global X Fix) ---
    # เราต้องมั่นใจว่า x ต่อเนื่องกัน ไม่ zig-zag
    plot_x = []
    plot_v = []
    plot_m = []
    current_offset = 0.0
    
    # วนลูปเพื่อสร้าง array x ที่ต่อเนื่อง
    for i in range(len(vis_spans)):
        span_data = df[df['span_id'] == i].copy()
        if span_data.empty: continue
        
        # เช็คว่าเป็น Local x หรือไม่ ถ้าใช่ให้บวก offset
        if i > 0 and span_data['x'].min() < 0.1:
            span_x = span_data['x'] + current_offset
        else:
            span_x = span_data['x']
            
        plot_x.extend(span_x.tolist())
        plot_v.extend(span_data['shear'].tolist())
        plot_m.extend((-span_data['moment']).tolist()) # Invert M for plotting logic
        current_offset += vis_spans[i]

    # --- ROW 2: SFD ---
    fig.add_trace(go.Scatter(x=plot_x, y=plot_v, mode='lines', line=dict(color='#1976D2', width=2), fill='tozeroy', fillcolor='rgba(25, 118, 210, 0.1)', name="Shear", hovertemplate="%{y:.2f}"), row=2, col=1)

    # --- ROW 3: BMD (Tension side / Sagging down) ---
    fig.add_trace(go.Scatter(x=plot_x, y=plot_m, mode='lines', line=dict(color='#D32F2F', width=2), fill='tozeroy', fillcolor='rgba(211, 47, 47, 0.1)', name="Moment", hovertemplate="%{y:.2f} (Inverted)"), row=3, col=1)
...
def draw_reinforcement_profile(spans, design_results, m_bar, s_bar):
    ...
    for i in range(len(spans)):
        ...
        # Bottom Steel (+M)
        if res.get('bot_nb'):
            txt_bot = f"{res['bot_nb']}-{m_bar}"
            fig.add_trace(go.Scatter(x=[x0 + 0.1, x1 - 0.1], y=[-0.35, -0.35], mode='lines+text', line=dict(color='#1565C0', width=5), text=[txt_bot, txt_bot], textposition="bottom center", showlegend=False, hoverinfo='text', hovertext=f"Bott: {txt_bot} (Mu={res['Mu_pos']:.2f})"))
        
        # Top Steel (-M)
        if res.get('top_nb'):
            txt_top = f"{res['top_nb']}-{m_bar}"
            fig.add_trace(go.Scatter(x=[x0, x1], y=[0.35, 0.35], mode='lines+text', line=dict(color='#D32F2F', width=3), text=[txt_top, txt_top], textposition="top center", showlegend=False, hoverinfo='text', hovertext=f"Top: {txt_top} (Mu={res['Mu_neg']:.2f})"))

        mid = (x0+x1)/2
        stir_txt = f"{s_bar} {res.get('stirrup_text', '-')}"
    ...
    fig.update_layout(title="🏗️ Reinforcement Profile (Side View)", height=300, xaxis=dict(range=[-0.5, total_len+0.5], showgrid=True), yaxis=dict(visible=False, range=[-1, 1]), margin=dict(l=20,r=20,t=40,b=20), plot_bgcolor='white')
    return fig
...
if st.session_state['analyzed'] and st.session_state.get('res_df') is not None:
    ...
    for i in range(n_span):
        # Filter Data per Span
        span_df = df[df['span_id'] == i]
        
        # หาค่า Max +M (Bottom), Max -M (Top), Max V
        m_max_pos = span_df['moment'].max()      # ค่าบวก = เหล็กล่าง
        m_max_neg = span_df['moment'].min()      # ค่าลบ = เหล็กบน
        v_max = span_df['shear'].abs().max()
        
        # Design Bottom (+M)
        res_bot = {'nb': 0}
        if m_max_pos > 0.01:
            res_bot = calculate_rc_design(m_max_pos, v_max, fc, fy, b_cm, h_cm, cov_cm, method, unit_sys, m_area, s_area, man_s_cm)
            
        # Design Top (-M) -> ส่งค่า absolute ไปคำนวณ
        res_top = {'nb': 0}
        if m_max_neg < -0.01:
            res_top = calculate_rc_design(abs(m_max_neg), v_max, fc, fy, b_cm, h_cm, cov_cm, method, unit_sys, m_area, s_area, man_s_cm)
            
        # Design Stirrup (ใช้ค่า Max Moment เพื่อเช็ค Shear)
        ...
        span_results.append({
            ...
            'stirrup_text': res_stir.get('stirrup_text', 'Err'),
            's_value_mm': res_stir.get('s_value_mm', 0)
        })

    # 3.3 PLOT REINFORCEMENT PROFILE (Updated)
    ...
    for i, col in enumerate(cols):
        ...
        with col:
            st.info(f"**Span {r['id']}**")
            st.write(f"👇 Bott: **{r['bot_nb']} - {m_bar}**")
            st.write(f"👆 Top: **{r['top_nb']} - {m_bar}**")
            st.write(f"⛓️ Stir: **{s_bar} {r['stirrup_text']}**")
            st.caption(f"Mu(+): {r['Mu_pos']:.2f} | Mu(-): {r['Mu_neg']:.2f}")
