...
# ตัด design_view ออก เพราะเราจะใช้ Matplotlib ในไฟล์นี้แทน
...
# =========================================================================
# 🛑 ORIGINAL PLOTTING ENGINE (RESTORED)
# กราฟวิเคราะห์แบบดั้งเดิม (Load, Shear, Moment, Deflection) ด้วย Matplotlib
# =========================================================================
def plot_analysis_results_original_style(res_df, spans, supports, loads, reactions):
    ...
    for i, s_type in enumerate(supports['type']):
        ...
        ax0.text(sup_x, -0.6, f"R{i}={r_val:.2f} kN", ha='center', va='top', fontsize=10, color='green', fontweight='bold')
    ...
    for _, load in loads.iterrows():
        if load['type'] == 'P': # Point Load
            ...
            ax0.text(x, dy, f"{mag:.2f} kN", ha='center', va='bottom' if mag>0 else 'top', color='red', fontsize=10)
        elif load['type'] == 'U': # Uniform Load
            x1 = load['d_start']
            x2 = x1 + load['dist']
            mag = load['mag']
            ax0.fill_between([x1, x2], [0, 0], [mag/max_mag, mag/max_mag], color='blue', alpha=0.3)
            ax0.text((x1+x2)/2, mag/max_mag, f"{mag:.2f} kN/m", ha='center', va='bottom', color='blue', fontsize=10)

    ax0.set_title("Load Diagram (kN, kN/m)", fontsize=12, fontweight='bold')
    ...
    ax1.text(0, v_max, f"{v_max:.2f}", color='blue', fontsize=9, ha='right')
    ax1.text(0, v_min, f"{v_min:.2f}", color='blue', fontsize=9, ha='right')
...
# --- 3. INTERNAL HELPER: PLOT CROSS SECTION (Style เดิมที่คุณต้องการ) ---
...
if not stable:
    ...
    try:
        ...
        with tab1:
            ...
            # ⚡ [RESTORED] Calling the Matplotlib Function instead of Plotly
            # ใช้ฟังก์ชันใหม่ที่เราสร้างขึ้นข้างบน
            loads_to_plot = calc_loads_ult if not is_service else calc_loads_svc
            fig = plot_analysis_results_original_style(
                res_df=df_for_plot, spans=spans, supports=sup_df, 
                loads=loads_to_plot, reactions=R_plot
            )
