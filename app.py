...
# ==========================================
# CUSTOM LOAD INPUT (FIXED LOGIC)
# ==========================================
def render_custom_load_input(n_span, spans, unit_sys, f_dl, f_ll):
    """
    แก้ไข: รับ f_dl, f_ll เข้ามาคูณให้เป็น Ultimate Load ก่อนส่งไปคำนวณ
    และส่ง Key ให้ครบทั้งตัวเล็กตัวใหญ่ (p, P, x, location) เพื่อกันพลาด
    """
            ...
            with c1:
                ...
                if w_u != 0:
                    loads.append({
                        ...
                        'type': 'U', 
                        'w_dl': w_dl, 'w_ll': w_ll, 
                        'w': w_u,      # ค่าที่ใช้คำนวณ (Factored)
                        'val': w_u     # สำรองเผื่อ Backend ใช้ key นี้
                    })
                ...
                for j in range(num_p):
                    ...
                    if p_u != 0:
                        loads.append({
                            ...
                            'type': 'P', 
                            'P_dl': p_dl, 'P_ll': p_ll, 
                            'P': p_u,       # Factored Load (Standard)
                            'p': p_u,       # Lowercase fallback
                            'val': p_u,     # Generic fallback
                            'x': x_loc,     # Standard Location
                            'location': x_loc # Fallback Location
                        })
    return loads
...
def create_engineering_plots(df, vis_spans, vis_supports, loads, unit_sys):
    ...
    # Intelligent Scaling
    w_vals = [abs(l['w']) for l in loads if l.get('type') == 'U' and l.get('w', 0) != 0]
    p_vals = [abs(l['P']) for l in loads if l.get('type') == 'P' and l.get('P', 0) != 0]
    ...
    fig = make_subplots(
        ...
        subplot_titles=("<b>Loading Diagram (Factored Loads)</b>", "<b>Shear Force Diagram (SFD)</b>", "<b>Bending Moment Diagram (BMD)</b>"),
    ...
    for r in [2, 3]:
        ...
        for x in cum_len: fig.add_vline(x=x, line_width=1, line_dash="dash", line_color="gray", opacity=0.5, row=r, col=1)

    fig.update_layout(height=800, showlegend=False, plot_bgcolor="white", hovermode="x unified", margin=dict(t=40, b=40, l=60, r=40))
...
# *** FIX: Pass factors to input function ***
...
if st.session_state['analyzed'] and st.session_state.get('res_df') is not None:
    ...
    # 3.4 DETAILED TABS (SECTION + LOGS)
            ...
            with c2:
                with st.expander(f"👇 Bottom Steel Calculation (+M)", expanded=True):
                    if r['bot_nb'] > 0:
                        ...
                        st.write("No positive moment significant enough for design.")
                
                with st.expander(f"👆 Top Steel Calculation (-M)", expanded=False):
                    if r['top_nb'] > 0:
                        for l in r['logs_top']: st.write(l)
                    else:
                        st.write("No negative moment significant enough for design.")
