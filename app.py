...
import rc_design  # ต้องเป็นไฟล์ที่แก้ด้านบน

st.set_page_config(page_title="Professional Beam Design", layout="wide")

# --- 1. Session State (ป้องกันข้อมูลหาย) ---
if 'loads' not in st.session_state: st.session_state.loads = []
if 'results' not in st.session_state: st.session_state.results = None

st.title("🏗️ Professional Beam Studio (Final Corrected)")
...
with st.container(border=True):
    ...
    with c1:
        ...
        spans = [st.number_input(f"L{i+1} (m)", 1.0, 20.0, 5.0, key=f"sp_{i}") for i in range(n_spans)]
    ...
    with c2:
        ...
        if st.button("➕ Add Load", use_container_width=True):
            ...
            st.session_state.loads.append({
                'span_index': l_idx, # Key ที่ถูกต้องสำหรับ Solver
                'type': l_type[0],   # 'P', 'U', 'M'
                'mag': l_mag * 1000, 
                'x': safe_x,
...
# --- 4. Display Results & Drawings ---
if st.session_state.results:
    df, reac, eq = st.session_state.results
    
    # 🔴 Safety Check: ตรวจสอบว่า Solver คืนค่ามาถูกต้องหรือไม่
    if df is None or df.empty:
        st.error("Solver returned empty data. Please check load positions.")
    else:
        # หาชื่อคอลัมน์ตำแหน่ง x (Solver บางตัวใช้ 'x' บางตัวใช้ 'dist')
        x_col = 'x' if 'x' in df.columns else df.columns[0]
        
        st.success("✅ Analysis Complete!")
        
        # --- A. LONGITUDINAL SECTION (รูปตัดยาว) ---
        ...
        # --- B. CROSS SECTIONS (รูปตัดขวางรายช่วง) ---
        ...
        for i in range(n_spans):
            with st.container(border=True):
                ...
                # Get Forces
                m_max = span_data['moment'].max() / 1000 # kNm
                m_min = span_data['moment'].min() / 1000 # kNm
                v_max = span_data['shear'].abs().max() / 1000 # kN
                
                # Call RC Design (ตอนนี้เรียกถูกชื่อแล้ว)
                ...
                with c1:
                    ...
                    st.markdown("---")
                    st.markdown(f"**🔴 Top Steel:** {res['neg']['n']} x DB{db_m} <small>({res['neg']['status']})</small>", unsafe_allow_html=True)
                    st.markdown(f"**🔵 Bot Steel:** {res['pos']['n']} x DB{db_m} <small>({res['pos']['status']})</small>", unsafe_allow_html=True)
