...
if not stable:
    ...
    try:
        ...
        # 5.2 Lists for Solver (Newton) and Graphics/Report (kN)
        solver_loads_list = []
        plot_loads_list = []
        ...
        if not loads_df.empty:
            for _, row in loads_df.iterrows():
                try:
                    ...
                    l_type = row['type']
                    mag_base_kN = float(row['mag']) # เช่น 3.6 kN
                    mag_factored_kN = mag_base_kN * f_ll
                    
                    # แปลงเป็น Newton สำหรับ Solver
                    mag_factored_N = mag_factored_kN * 1000.0 
                    dist = float(row['dist']) 
                    
                    # เก็บค่าลง Solver List (N)
                    solver_loads_list.append({
                        'span_index': s_idx, 'type': l_type, 'mag': mag_factored_N, 'dist': dist
                    })
                    
                    # เก็บค่าลง Plot List (kN) - ป้องกัน 3600 ในรูปและรายงาน
                    plot_loads_list.append({
                        'span_index': s_idx, 'type': l_type, 'mag': mag_factored_kN, 
                        'dist': dist, 'desc': 'User Load'
                    })
                except Exception: continue
        
        # 5.4 Merge Self-Weight (UDL)
        for i in range(n_spans):
            # เข้า Solver (N/m)
            solver_loads_list.append({
                'span_index': i, 'type': 'U', 'mag': w_sw_factored_kN * 1000.0, 'dist': spans[i]
            })
            # เข้า Plot (kN/m)
            plot_loads_list.append({
                'span_index': i, 'type': 'U', 'mag': w_sw_factored_kN, 
                'dist': spans[i], 'desc': 'Total UDL'
            })
        
        calc_loads_df = pd.DataFrame(solver_loads_list)
        plot_loads_df = pd.DataFrame(plot_loads_list)

        # --- 6. BEAM SOLVER ---
        x_eval, M, V, D, R = solver.solve_beam(spans, sup_df, calc_loads_df, params)
        
        # ผลลัพธ์สำหรับแสดงผล (แปลงจาก N เป็น kN)
        res_df = pd.DataFrame({
            'x': x_eval,
            'moment': M / 1000.0,
            'shear': V / 1000.0,
        ...
        with tab1:
            ...
            with st.expander("🧮 Detailed Load Calculation Report", expanded=True):
                ...
                sw_rep = [{"Span": i+1, "Formula": f"b*h*24*f_dl", "Factored": f"{w_sw_factored_kN:.2f} kN/m"} for i in range(n_spans)]
                ...
                # SW
                for i in range(n_spans):
                    combo_rep.append({"Span": i+1, "Type": "Self-Weight (DL)", "Unfactored": f"{w_sw_base_kN:.2f}", "Factor": f"x{f_dl}", "Factored": f"{w_sw_factored_kN:.2f} kN/m"})
                # User Loads (ใช้หน่วย kN เสมอในตารางนี้)
                if not loads_df.empty:
                    for _, row in loads_df.iterrows():
                        u_mag = float(row['mag'])
                        combo_rep.append({
                            "Span": int(row['span_index'])+1,
                            "Type": f"{'Point' if row['type'] == 'P' else 'Uniform'} (LL)",
                            "Unfactored": f"{u_mag:.2f}",
                            "Factor": f"x{f_ll}",
                            "Factored": f"{(u_mag * f_ll):.2f} kN(/m)"
                        })
                st.table(pd.DataFrame(combo_rep))
                ...
                with ec1:
                    ...
                    if not loads_df.empty:
                        for _, r in loads_df.iterrows():
                            # คำนวณเป็น kN เพื่อเช็คสมดุล
                            total_app += (float(r['mag']) * f_ll) * (1.0 if r['type'] == 'P' else float(r['dist']))
                    st.write(f"Total Reactions: **{sum_R:.2f} kN**")
                    st.write(f"Total Applied: **{total_app:.2f} kN**")
                    if abs(sum_R - total_app) < 0.5: st.success("Balance Check: PASS")
        ...
        with tab2:
            ...
            for i, span_len in enumerate(spans):
                ...
                if not span_data.empty:
                    mu_p, mu_n = span_data['moment'].max(), abs(span_data['moment'].min())
                    ...
                    s_req, _, stp_s = rc_design.check_shear(vu_max, params['b'], d_eff, params['fc'], params['fy'])
                    
                    def n_bars(As): return max(2, int(np.ceil(As / (np.pi * 0.008**2)))) 
                    
                    design_res.append({'span': i+1, 'pos': {'n': n_bars(As_p)}, 'neg': {'n': n_bars(As_n)}, 'shear': {'s': s_req}})
                    
                    with st.expander(f"📘 Detailed Design: Span {i+1}"):
                        st.markdown(f"**Flexural Design (Mu+ = {mu_p:.2f}, Mu- = {mu_n:.2f} kNm)**")
                        for s in stp_p: st.latex(s)
                        st.markdown("**Shear Design**")
                        for s in stp_s: st.latex(s)
                span_start += span_len
            ...
            if design_res:
                ...
                with c1: st.pyplot(section_plotter.plot_section(params['b'], params['h'], 40, 16, design_res[0]['neg']['n'], design_res[0]['pos']['n'], "RB6@200", params['fc'], params['fy']))
                with c2: st.pyplot(section_plotter.plot_longitudinal_section_detailed(spans, sup_df, design_res, params['h'], 40))
