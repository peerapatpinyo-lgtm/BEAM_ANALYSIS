...
if 'avg_factor' not in st.session_state: st.session_state.avg_factor = 1.6 # Default fallback
...
with st.sidebar:
    ...
    # Define Current Factors based on selection
...
if run_btn:
    if not stable:
        ...
        st.session_state.avg_factor = current_avg_factor # Save the factor used for analysis
...
if st.session_state.analyzed:
    ...
    # [FIX] Use .get() to safely retrieve avg_factor, prevent crash if state is stale
    saf_factor = st.session_state.get('avg_factor', current_avg_factor)
