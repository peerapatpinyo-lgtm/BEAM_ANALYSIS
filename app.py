...
if not stable:
    ...
    # --- [FEATURE] ANALYSIS SETTINGS (ULTIMATE/SERVICE) ---
    ...
    try:
        ...
        # ================= TAB 1: ANALYSIS RESULTS (คืนค่ากราฟและ Metric) =================
        with tab1:
            st.subheader(f"📈 Structural Analysis Diagrams ({tag} Load)")
        ...
        # ================= TAB 2: INTERACTIVE DESIGN (ลำดับ Top -> Bottom -> Shear) =================
        with tab2:
            ...
            for i in range(n_spans):
                ...
                with st.expander(f"📍 SPAN {i+1} (L={s_len} m)", expanded=True):
                    ...
                    with col_input:
                        ...
                        # --- CALCULATION LOGIC ---
                        d_t = h_mm - (cover_mm + stir_db + t_db/2 + (t_lay-1)*25)
                        ...
                        d_b = h_mm - (cover_mm + stir_db + b_db/2 + (b_lay-1)*25)
                        ...
                        # --- DISPLAY TABLES (Markdown ครบถ้วน) ---
                        st.markdown(f"**Top Steel Analysis:**")
                        st.markdown(f"| Parameter | Calc As ($M_u$) | Min $A_{{s,min}}$ | Provided As | Status |")
                        st.markdown(f"| :--- | :--- | :--- | :--- | :--- |")
                        st.markdown(f"| Steel Area | {as_req_t:.0f} mm² | {as_min_t:.0f} mm² | **{as_prov_t:.0f} mm²** | {'✅' if as_prov_t >= max(as_req_t, as_min_t) else '❌'} |")
                        st.markdown(f"| Capacity | $M_u$: {mu_neg:.1f} | - | **$\phi M_n$: {phi_Mn_t:.1f}** | {'✅' if phi_Mn_t >= mu_neg else '❌'} |")

                        st.markdown(f"**Bottom Steel Analysis:**")
                        st.markdown(f"| Parameter | Calc As ($M_u$) | Min $A_{{s,min}}$ | Provided As | Status |")
                        st.markdown(f"| :--- | :--- | :--- | :--- | :--- |")
                        st.markdown(f"| Steel Area | {as_req_b:.0f} mm² | {as_min_t:.0f} mm² | **{as_prov_b:.0f} mm²** | {'✅' if as_prov_b >= max(as_req_b, as_min_t) else '❌'} |")
