...
def add_peak_annotations(fig, x_data, y_data, row, col):
    """Safe annotation wrapper that forces float conversion."""
    try:
        idx_max, idx_min = np.argmax(y_data), np.argmin(y_data)
        val_max, val_min = float(y_data[idx_max]), float(y_data[idx_min])
        x_max, x_min = float(x_data[idx_max]), float(x_data[idx_min])
...
def draw_diagrams(df, spans, supports, loads, u_force, u_len):
    ...
    for i, x in enumerate(cum_len):
        ...
        if stype == "Pin":
            ...
            # Engineering Style Fixed Support
    ...
    for l in loads:
        if l['type'] == 'U':
            ...
            fig.add_annotation(x=(x1+x2)/2, y=h, text=f"Wu={float(l['w']):.0f}", showarrow=False, yshift=15, font=dict(color="#0D47A1", size=10), row=1, col=1)
        elif l['type'] == 'P':
            px = cum_len[l['span_idx']] + float(l['x'])
            fig.add_annotation(x=px, y=0, ax=0, ay=-50, text=f"Pu={float(l['P']):.0f}", arrowhead=2, arrowwidth=2, arrowcolor="#D32F2F", font=dict(color="#D32F2F", size=10), row=1, col=1)
...
def render_combined_section(b, h, cover, top_bars, bot_bars):
    ...
    draw_rebars(bot_bars, cover + 1.5, "#1565C0", 5) 
    draw_rebars(top_bars, h - cover - 1.5, "#C62828", -5)
...
def render_design_results(df, params, spans, span_props_list, supports):
    ...
    for i, tab in enumerate(tabs):
        ...
        with tab:
            st.markdown(f"**Section Properties:** {float(b_s):.0f} x {float(h_s):.0f} cm (Cov: {float(cv_s):.1f} cm)")
