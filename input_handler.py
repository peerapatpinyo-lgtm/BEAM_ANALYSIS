...
def render_sidebar():
    with st.sidebar:
        ...
        E = st.number_input("E (Elastic Modulus) [MPa]", value=25000.0) * 1e6 
...
def render_model_inputs(params):
    ...
    for i in range(n_spans):
        with cols[i%5]:
            s = st.number_input(f"Span {i+1} (m)", value=5.0, key=f"s_{i}")
            spans.append(s)
    ...
    for i, c in enumerate(cols):
        with c:
            st.caption(f"Node {i} (x={cum_dist[i]:.1f})")
    ...
    # Basic check: needs at least 2 supports or 1 fixed
    stable = True if fixed_count >= 2 or (fixed_count >=1 and sup_data[0]['type']=='Fixed') else True # Simplified check
...
def render_loads(n_spans, spans, params, sup_df):
    ...
    with st.expander("➕ Add New Load", expanded=True):
        ...
        span_len = spans[s_idx]
        lx = c3.number_input("Start x (m)", min_value=0.0, max_value=span_len, value=0.0)
        mag = c4.number_input("Magnitude (kN, kNm)", value=10.0)
        case = c5.selectbox("Case", ["DL", "LL"])
        
        dist = 0.0
        if "Uniform" in l_type:
            dist = st.number_input("Length (m) for UDL", min_value=0.1, max_value=span_len-lx, value=span_len-lx)
        ...
        if st.button("Add Load"):
            st.session_state.load_list.append({
                ...
                "span_index": s_idx, 
                "x": lx, 
                "mag": mag*1000, # Convert to N here
                "dist": dist, 
                "case": case
            })
    ...
    if st.session_state.load_list:
        ...
        # Show table with kN units
        display_df = df.copy()
        display_df['mag'] = display_df['mag'] / 1000
        st.dataframe(display_df[["type", "span_index", "x", "mag", "dist", "case"]], use_container_width=True)
