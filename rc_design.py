...
def design_span_expert(mu_pos, mu_neg, vu, b_m, h_m, fc, fy, fyt, cover_mm, db_main, db_stirrup):
    ...
    def calc_flexure(mu_knm):
        if abs(mu_knm) < 0.1: 
            return {"n": 2, "as_req": 0, "status": "Min Steel", "et": 0.005, "a": 0, "mu_val": mu_knm}
        mu_n = (abs(mu_knm) * 1e6) / phi_m
        k = mu_n / (b * d**2)
        m = fy / (0.85 * fc)
        check_val = 1 - (2 * m * k / fy)
        if check_val < 0: return {"status": "FAIL: OVER-REINFORCED", "n": 0, "mu_val": mu_knm}
        ...
        et = ((d - (a/0.85)) / (a/0.85)) * 0.003
        n_bars = max(2, int(np.ceil(as_req / (np.pi * (db_main**2) / 4))))
        return {"n": n_bars, "as_req": as_req, "a": a, "et": et, "status": "OK", "mu_val": mu_knm}

    vc = (0.17 * np.sqrt(fc) * b * d) / 1000
    return {"pos": calc_flexure(mu_pos), "neg": calc_flexure(mu_neg), "d": d, "vu": vu, "phi_vc": phi_v * vc}
