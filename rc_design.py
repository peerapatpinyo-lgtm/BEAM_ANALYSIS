...
def calculate_rc_design(max_M, max_V, fc, fy, b, h, cov, method, unit_sys, main_bar_area, stirrup_area, manual_s=0):
    ...
    # Input b, h come in as cm (from input_handler conversion)
    logs.append(f"- **Section:** $b={b*10:.0f}$ mm, $h={h*10:.0f}$ mm") 

    # --- 2. Unit Conversion (Internal Calc uses kg, cm) ---
    if "kN" in unit_sys:
    ...
    if result.get('As_req', 0) == 9999:
        ...
        nb_use = max(2, int(np.ceil(nb_calc))) # Minimum 2 bars
        result['nb'] = nb_use
    ...
    if Vu_calc > Vc:
        ...
        if manual_s > 0:
            ...
            # Rounding logic for cm -> then convert to mm later
            limit_s = min(s_req_cm, d/2, 60)
            if limit_s < 2.5: limit_s = 2.5
            s_use_cm = int(5 * round(limit_s/5)) # Round to nearest 5cm usually, or use step logic
            if s_use_cm == 0: s_use_cm = 5
            if s_use_cm > limit_s: s_use_cm = int(limit_s) # Cap at limit
            
        logs.append(f"- $V_u > V_c$: Need stirrups.")
        logs.append(f"- Theoretical spacing: {s_req_cm*10:.0f} mm")
    else:
    ...
    # Round logic for standard construction (e.g. 100, 125, 150, 200 mm)
    # But for now, let's stick to the cm calculation * 10 to match engineering habits (e.g. @200mm)
