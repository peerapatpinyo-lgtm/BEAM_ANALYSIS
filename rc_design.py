...
def design_span_expert(m_pos, m_neg, v_u, b, h, fc, fy, cover, db):
    ...
    def get_steel(Mu_kNm, is_top):
        ...
        # คำนวณกำลังรับโมเมนต์จริงของหน้าตัดที่ใส่เหล็กไปแล้ว (n เส้น)
        As_provided = n * A_bar
    ...
    return {
        "pos": {
            ...
            "capacity": cap_pos # <--- ส่งค่า Capacity กลับไป
        },
        "neg": {
            "n": n_neg, "note": note_neg, 
            "Ld": Ld_neg, "Ls": Ls_neg, 
            "crack_ok": cr_neg, "s_limit": s_lim_neg,
            "capacity": cap_neg # <--- ส่งค่า Capacity กลับไป
        },
