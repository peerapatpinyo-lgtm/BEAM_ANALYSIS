...
def design_span(mu_pos, mu_neg, vu, b_m, h_m, fc, fy, fyt, cover_mm, db_main, db_stirrup):
    phi_m, phi_v = 0.90, 0.75
    b, h = b_m * 1000, h_m * 1000
    d = h - cover_mm - db_stirrup - (db_main / 2)
    
    def calc_flexure(mu_knm):
        if abs(mu_knm) < 0.1: return {"n": 2, "as_req": 0, "k": 0, "rho": 0, "status": "Minimum Steel"}
        mu_n = (abs(mu_knm) * 1e6) / phi_m
        k = mu_n / (b * d**2)
        m = fy / (0.85 * fc)
        check_val = 1 - (2 * m * k / fy)
        
        if check_val < 0: return {"n": 0, "as_req": 0, "k": k, "rho": 0, "status": "SECTION TOO SMALL"}
        ...
        return {"n": n_bars, "as_req": as_req, "k": k, "rho": rho_final, "status": "OK"}
