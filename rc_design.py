...
def design_section_comprehensive(mu_pos, mu_neg, vu, b_m, h_m, fc, fy, fyt, cover_mm, db_main, db_stirrup):
    phi_m, phi_v = 0.90, 0.75
    b, h = b_m * 1000, h_m * 1000
    d = h - cover_mm - db_stirrup - (db_main / 2)
    
    def calc_detailed(mu_knm):
        if abs(mu_knm) < 0.1: 
            return {"n": 2, "as_req": 0, "k": 0, "rho": 0, "a": 0, "c": 0, "status": "Min Steel"}
        ...
        if check_val < 0:
            return {"n": 0, "as_req": 0, "k": k, "rho": 0, "status": "FAIL: SECTION TOO SMALL"}
        ...
        # Flexural calculation details
        a = (as_req * fy) / (0.85 * fc * b) # Depth of compression block
        beta1 = max(0.65, 0.85 - 0.05 * (fc - 28) / 7)
        c = a / beta1 # Neutral axis depth
        
        n_bars = max(2, int(np.ceil(as_req / (np.pi * (db_main**2) / 4))))
        return {"n": n_bars, "as_req": as_req, "k": k, "rho": rho_final, "a": a, "c": c, "status": "OK"}

    res_pos = calc_detailed(mu_pos)
    res_neg = calc_detailed(mu_neg)
