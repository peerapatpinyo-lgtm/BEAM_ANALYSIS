...
def plot_as_svg(spans, sup_df, design_res, h_m, cover_mm):
    ...
    เพื่อให้ได้ความคมชัดสูงสุด ซูมไม่แตก และสัดส่วนถูกต้องตามหลักวิศวกรรม
    """
    spans_mm = [s * 1000 for s in spans]
    total_L = sum(spans_mm)
    v_h = 350  # บังคับความสูงคานให้เพรียวบางระดับสากล
    ...
    # 4. วาด Supports ตามประเภท (Engineering Symbols)
    if not sup_df.empty:
