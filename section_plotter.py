...
# ใช้ SCALE_FACTOR อ้างอิงเพื่อให้ขนาดทางกายภาพของวัตถุใกล้เคียงกัน
SCALE_FACTOR = 800 
...
def plot_section(b_m, h_m, cover_mm, db_main_mm, n_top, n_bottom, stirrup_name, fc, fy):
    ...
    # Viewport กลับมาเท่าเดิม (ซูมเข้า)
    view_left, view_right = -200, b + 500
    view_bottom, view_top = -250, h + 250
    
    width_inches = (view_right - view_left) / SCALE_FACTOR 
    height_inches = (view_top - view_bottom) / SCALE_FACTOR
    
    # เพิ่ม DPI เป็น 300 เพื่อความคมชัดสูงสุด (แก้ปัญหาภาพแตก)
    fig, ax = plt.subplots(figsize=(width_inches, height_inches), dpi=300)
    
    # คอนกรีต (lw=1.5 เพื่อความชัด) และเหล็กปลอก
    ax.add_patch(patches.Rectangle((0, 0), b, h, linewidth=1.5, edgecolor='#000000', facecolor='#ffffff', antialiased=True))
    ax.add_patch(patches.Rectangle((cover, cover), b-2*cover, h-2*cover, linewidth=0.8, edgecolor='#4d5656', facecolor='none', antialiased=True))
    ...
    def draw_bars(n, y_pos, color):
        ...
        for i in range(int(n)):
            ...
            ax.add_patch(plt.Circle((x, y_pos), db/2, color=color, zorder=10, antialiased=True))

    y_bot, y_top = cover + ds + db/2, h - cover - ds - db/2
    draw_bars(n_bottom, y_bot, '#a93226')
    draw_bars(n_top, y_top, '#1f618d')
    
    # ตัวหนังสือเล็กลง (7) และคมชัด
    f_size = 7
    ...
    ax.text(b/2, h + 80, f"{stirrup_name}", ha='center', color='#1d8348', fontweight='bold', fontsize=f_size)

    # Dim Ticks เส้นบางแต่คม
    def draw_tick_dim(p1, p2, text, vert=False):
        ax.plot([p1[0], p2[0]], [p1[1], p2[1]], color='#000000', lw=0.5, antialiased=True)
        tick = 15
        for p in [p1, p2]:
            ax.plot([p[0]-tick, p[0]+tick], [p[1]-tick, p[1]+tick], color='#000000', lw=0.8, antialiased=True)
        if vert:
            ax.text(p1[0]-50, (p1[1]+p2[1])/2, text, va='center', ha='right', rotation=90, fontsize=f_size-1)
        else:
            ax.text((p1[0]+p2[0])/2, p1[1]-50, text, ha='center', va='top', fontsize=f_size-1)
...
def plot_longitudinal_section_detailed(spans, sup_df, design_res, h_m, cover_mm):
    ...
    fig, ax = plt.subplots(figsize=(width_inches, height_inches), dpi=300) # DPI 300
    
    ax.add_patch(patches.Rectangle((0, 0), total_L, h_beam, linewidth=1.5, edgecolor='#000000', facecolor='#ffffff', antialiased=True))
    
    for _, sup in sup_df.iterrows():
        sx = sup['x'] * 1000
        stype = sup['type']
        if stype == 'Fixed':
            ax.add_patch(patches.Rectangle((sx-20, -150), 40, h_beam+300, facecolor='#f2f4f4', hatch='///', alpha=0.5, edgecolor='#7f8c8d', lw=0.5))
        elif stype == 'Pin':
            poly = plt.Polygon([[sx-100, -120], [sx+100, -120], [sx, 0]], closed=True, facecolor='none', edgecolor='#000000', lw=0.8)
            ax.add_patch(poly)
            ax.plot([sx-120, sx+120], [-122, -122], color='black', lw=0.8)
        elif stype == 'Roller':
            ...
            ax.plot([sx-120, sx+120], [-160, -160], color='black', lw=0.8)

    f_size = 7 # เล็กลงเท่ากัน
