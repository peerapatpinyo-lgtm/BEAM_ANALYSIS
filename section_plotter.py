...
def plot_section(b, h, cover_mm, db_mm, n_top, n_bot, stirrup_info, fc=None, fy=None):
    ...
    """
    # Create Figure
    fig, ax = plt.subplots(figsize=(5, 5)) # Slightly larger
    
    # 1. Concrete Section
    rect = patches.Rectangle((0, 0), b, h, linewidth=2, edgecolor='#2C3E50', facecolor='#ECF0F1')
    ax.add_patch(rect)
    ...
    ax.text(b+0.05, h/2, stir_lbl, rotation=270, va='center', color='#C0392B', fontsize=10)

    # 5. Material Specs Box (New Feature)
    if fc and fy:
        spec_text = f"Materials:\n$f'_c$ = {fc:.0f} MPa\n$f_y$ = {fy:.0f} MPa"
        ax.text(b*1.5, h*0.9, spec_text, fontsize=9, bbox=dict(boxstyle="round", facecolor='white', alpha=0.8))

    # Settings
    ax.set_xlim(-0.15, b + 0.25)
