...
class BeamSolver:
    def __init__(self, spans, supports_input, loads_input, E, I, A=None, G=None):
        ...
        # 1. สร้างพิกัด Node หลัก (Span Ends)
    ...
    def _sanitize_supports(self, supports_input):
        sanitized = []
        # แปลงเป็น list ของ dict ไม่ว่า input จะเป็น DataFrame หรือ List
        data = supports_input.to_dict('records') if hasattr(supports_input, 'to_dict') else supports_input
        
        for s in data:
            # ดึง ID (ลองทุก Key ที่อาจเป็นไปได้จาก Streamlit)
            raw_id = s.get('id', s.get('Node ID'))
            if raw_id is None: continue
            
            idx = int(raw_id)
            # ถ้ามาจาก Node ID ใน Editor (1, 2, 3) ให้ปรับเป็น Index (0, 1, 2)
    ...
    def solve(self):
        # --- สร้าง UNIQUE NODES (ใช้ Tolerance ป้องกัน Node ซ้อน) ---
        ...
        for _, l in self.loads_df.iterrows():
            if l['type'] in ['P', 'M']:
                ...
                for i in range(num_nodes - 1):
                    ...
                    if overlap > 1e-6:
                        # กระจายแรงเข้า Nodal Force (Consistent Loading)
                        F[2*i] -= l['mag'] * overlap * 0.5
                        F[2*(i+1)] -= l['mag'] * overlap * 0.5

        # 3. Boundary Conditions (Support Constraints)
        ...
        # 6. INTERNAL FORCES (Statics)
        results = []
        # เก็บพิกัด Node เพื่อความแม่นยำของจุดหักเห
        plot_x = np.unique(np.concatenate([np.linspace(0, nodes[-1], 350), nodes]))
        for x in plot_x:
            V, M, defl = 0.0, 0.0, 0.0
            # Shear & Moment โดยการตัด Section จากซ้าย
